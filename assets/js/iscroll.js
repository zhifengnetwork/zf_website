...
};m.removeEvent=function(r,q,p,o){r.removeEventListener(q,p,!!o);};m.prefixPointerEvent=function(o){return f.MSPointerEvent?"MSPointer"+o.charAt(9).toUpperCase()+o.substr(10):o;
};m.momentum=function(u,q,r,o,v,w){var p=u-q,s=e.abs(p)/r,x,t;w=w===undefined?0.0006:w;x=u+(s*s)/(2*w)*(p<0?-1:1);t=s/w;if(x<o){x=v?o-(v/2.5*(s/8)):o;p=e.abs(x-u);
t=p/s;}else{if(x>0){x=v?v/2.5*(s/8):0;p=e.abs(u)+x;t=p/s;}}return{destination:e.round(x),duration:t};};var j=l("transform");m.extend(m,{hasTransform:j!==false,hasPerspective:l("perspective") in n,hasTouch:"ontouchstart" in f,hasPointer:f.PointerEvent||f.MSPointerEvent,hasTransition:l("transition") in n});
m.isBadAndroid=/Android /.test(f.navigator.appVersion)&&!(/Chrome\/\d/.test(f.navigator.appVersion));m.extend(m.style={},{transform:j,transitionTimingFunction:l("transitionTimingFunction"),transitionDuration:l("transitionDuration"),transitionDelay:l("transitionDelay"),transformOrigin:l("transformOrigin")});
...
m.extend(m.ease={},{quadratic:{style:"cubic-bezier(0.25, 0.46, 0.45, 0.94)",fn:function(o){return o*(2-o);}},circular:{style:"cubic-bezier(0.1, 0.57, 0.1, 1)",fn:function(o){return e.sqrt(1-(--o*o));
...
}}}}},elastic:{style:"",fn:function(o){var p=0.22,q=0.4;if(o===0){return 0;}if(o==1){return 1;}return(q*e.pow(2,-10*o)*e.sin((o-p/4)*(2*e.PI)/p)+1);}}});
m.tap=function(q,o){var p=a.createEvent("Event");p.initEvent(o,true,true);p.pageX=q.pageX;p.pageY=q.pageY;q.target.dispatchEvent(p);};m.click=function(r){var q=r.target,p;
var v=($(q).index())*$(q).height();var t=$(q).closest(".wheel").index();var s=$(q).closest(".number_select");var o=parseInt($(q).closest("li").text());
$(q).addClass("cur").siblings().removeClass("cur");$(".wheel").removeClass("cur_wheel");$(q).closest(".wheel").addClass("cur_wheel");if(isNaN(o)){var u=parseInt($(".cur_wheel ul li .cur").text());
if(u>=0){o=u;}else{o=0;}}$(".s_bar_c .show_num").text(o);if(!(/(SELECT|INPUT|TEXTAREA)/i).test(q.tagName)){p=a.createEvent("MouseEvents");p.initMouseEvent("click",true,true,r.view,1,q.screenX,q.screenY,q.clientX,q.clientY,r.ctrlKey,r.altKey,r.shiftKey,r.metaKey,0,null);
p._constructed=true;q.dispatchEvent(p);}};return m;})();function g(l,j){this.wrapper=typeof l=="string"?a.querySelector(l):l;this.scroller=this.wrapper.children[0];
this.scrollerStyle=this.scroller.style;this.options={resizeScrollbars:true,mouseWheelSpeed:20,snapThreshold:0.334,startX:0,startY:0,scrollY:true,directionLockThreshold:5,momentum:true,bounce:true,bounceTime:600,bounceEasing:"",preventDefault:true,preventDefaultException:{tagName:/^(INPUT|TEXTAREA|BUTTON|SELECT)$/},HWCompositing:true,useTransition:true,useTransform:true};
...
}this.options.invertWheelDirection=this.options.invertWheelDirection?-1:1;this.x=0;this.y=0;this.directionX=0;this.directionY=0;this._events={};this._init();
this.refresh();this.scrollTo(this.options.startX,this.options.startY);this.enable();}g.prototype={version:"5.1.3",_init:function(){this._initEvents();if(this.options.scrollbars||this.options.indicators){this._initIndicators();
}if(this.options.mouseWheel){this._initWheel();}if(this.options.snap){this._initSnap();}if(this.options.keyBindings){this._initKeys();}},destroy:function(){this._initEvents(true);
this._execEvent("destroy");},_transitionEnd:function(i){if(i.target!=this.scroller||!this.isInTransition){return;}this._transitionTime();if(!this.resetPosition(this.options.bounceTime)){this.isInTransition=false;
...
}if(this.options.preventDefault&&!c.isBadAndroid&&!c.preventDefaultException(j.target,this.options.preventDefaultException)){j.preventDefault();}var i=j.touches?j.touches[0]:j,k;
this.initiated=c.eventType[j.type];this.moved=false;this.distX=0;this.distY=0;this.directionX=0;this.directionY=0;this.directionLocked=0;this._transitionTime();
this.startTime=c.getTime();if(this.options.useTransition&&this.isInTransition){this.isInTransition=false;k=this.getComputedPosition();this._translate(e.round(k.x),e.round(k.y));
this._execEvent("scrollEnd");}else{if(!this.options.useTransition&&this.isAnimating){this.isAnimating=false;this._execEvent("scrollEnd");}}this.startX=this.x;
this.startY=this.y;this.absStartX=this.x;this.absStartY=this.y;this.pointX=i.pageX;this.pointY=i.pageY;this._execEvent("beforeScrollStart");},_move:function(p){if(!this.enabled||c.eventType[p.type]!==this.initiated){return;
}if(this.options.preventDefault){p.preventDefault();}var t=p.touches?p.touches[0]:p,l=t.pageX-this.pointX,k=t.pageY-this.pointY,q=c.getTime(),i,u,o,n;this.pointX=t.pageX;
this.pointY=t.pageY;this.distX+=l;this.distY+=k;o=e.abs(this.distX);n=e.abs(this.distY);if(q-this.endTime>300&&(o<10&&n<10)){return;}if(!this.directionLocked&&!this.options.freeScroll){if(o>n+this.options.directionLockThreshold){this.directionLocked="h";
}else{if(n>=o+this.options.directionLockThreshold){this.directionLocked="v";}else{this.directionLocked="n";}}}if(this.directionLocked=="h"){if(this.options.eventPassthrough=="vertical"){p.preventDefault();
}else{if(this.options.eventPassthrough=="horizontal"){this.initiated=false;return;}}k=0;}else{if(this.directionLocked=="v"){if(this.options.eventPassthrough=="horizontal"){p.preventDefault();
}else{if(this.options.eventPassthrough=="vertical"){this.initiated=false;return;}}l=0;}}l=this.hasHorizontalScroll?l:0;k=this.hasVerticalScroll?k:0;i=this.x+l;
u=this.y+k;if(i>0||i<this.maxScrollX){i=this.options.bounce?this.x+l/3:i>0?0:this.maxScrollX;}if(u>0||u<this.maxScrollY){u=this.options.bounce?this.y+k/3:u>0?0:this.maxScrollY;
}this.directionX=l>0?-1:l<0?1:0;this.directionY=k>0?-1:k<0?1:0;if(!this.moved){this._execEvent("scrollStart");}this.moved=true;this._translate(i,u);if($(p.target).closest(".s_bar_c").length>0){var s=$(".s_bar_c .show_num").text();
var r=parseInt(s);if(isNaN(r)){var m=$(".cur_wheel ul li .cur").text();var j=parseInt(m);if(j>=0){r=j;}else{r=0;}}r=r-(k);r=parseInt((r<0)?0:r);r=parseInt((r>100)?100:r);
$(".s_bar_c .show_num").text(r);$(".cur_wheel .cur").text(r);$(".cur_wheel .cur").prev().text(r-1);$(".cur_wheel .cur").prev().prev().text(r-2);$(".cur_wheel .cur").next().text(r+1);
$(".cur_wheel .cur").next().next().text(r+2);}if($(p.target).closest(".s_circle_c").length>0){var r=parseInt($(".s_circle_c .show_num").text());r=r-(k);
r=parseInt((r<0)?0:r);r=parseInt((r>240)?240:r);$(".s_circle_c .show_num").text(r);}if(q-this.startTime>300){this.startTime=q;this.startX=this.x;this.startY=this.y;
}},_end:function(o){if(!this.enabled||c.eventType[o.type]!==this.initiated){return;}if(this.options.preventDefault&&!c.preventDefaultException(o.target,this.options.preventDefaultException)){o.preventDefault();
...
i=k.destination;t=j.destination;l=e.max(k.duration,j.duration);this.isInTransition=1;}if(this.options.snap){var m=this._nearestSnap(i,t);this.currentPage=m;
l=this.options.snapSpeed||e.max(e.max(e.min(e.abs(i-m.x),1000),e.min(e.abs(t-m.y),1000)),300);i=m.x;t=m.y;this.directionX=0;this.directionY=0;p=this.options.bounceEasing;
...
},refresh:function(){var i=this.wrapper.offsetHeight;this.wrapperWidth=this.wrapper.clientWidth;this.wrapperHeight=this.wrapper.clientHeight;this.scrollerWidth=this.scroller.offsetWidth;
this.scrollerHeight=this.scroller.offsetHeight;this.maxScrollX=this.wrapperWidth-this.scrollerWidth;this.maxScrollY=this.wrapperHeight-this.scrollerHeight;
...
l=this.y+l;j=j||0;this.scrollTo(i,l,j,k);},scrollTo:function(i,n,l,m){m=m||c.ease.circular;this.isInTransition=this.options.useTransition&&l>0;if(!l||(this.options.useTransition&&m.style)){this._transitionTimingFunction(m.style);
this._transitionTime(l);var j=$(this.wrapper).find("li").height()*(-1);var k=e.round(n/j);if(n%j!=0){n=j*k;}this._translate(i,n);}else{this._animate(i,n,l,m.fn);
}},scrollToElement:function(j,k,i,n,m){j=j.nodeType?j:this.scroller.querySelector(j);if(!j){return;}var l=c.offset(j);l.left-=this.wrapperOffset.left;l.top-=this.wrapperOffset.top;
if(i===true){i=e.round(j.offsetWidth/2-this.wrapper.offsetWidth/2);}if(n===true){n=e.round(j.offsetHeight/2-this.wrapper.offsetHeight/2);}l.left-=i||0;
l.top-=n||0;l.left=l.left>0?0:l.left<this.maxScrollX?this.maxScrollX:l.left;l.top=l.top>0?0:l.top<this.maxScrollY?this.maxScrollY:l.top;k=k===undefined||k===null||k==="auto"?e.max(e.abs(this.x-l.left),e.abs(this.y-l.top)):k;
this.scrollTo(l.left,l.top,k,m);},_transitionTime:function(k){k=k||0;this.scrollerStyle[c.style.transitionDuration]=k+"ms";if(!k&&c.isBadAndroid){this.scrollerStyle[c.style.transitionDuration]="0.001s";
...
if(this.indicators){for(var j=this.indicators.length;j--;){this.indicators[j].transitionTimingFunction(k);}}},_translate:function(j,p){if(this.options.useTransform){this.scrollerStyle[c.style.transform]="translate("+j+"px,"+p+"px)"+this.translateZ;
}else{j=e.round(j);p=e.round(p);this.scrollerStyle.left=j+"px";this.scrollerStyle.top=p+"px";if(j<0){$(".toptitle").addClass("hasarrowl");var k=$(".toptitle .iscroll").width();
var m=$(".toptitle .scroller").width();var o=$(".topops-addbtn a").outerWidth(true);if(m-o-k+j<=0){$(".toptitle").removeClass("hasarrowr");}else{$(".toptitle").addClass("hasarrowr");
}}if(j>0){$(".toptitle").removeClass("hasarrowl");var m=$(".toptitle .scroller").width();var k=$(".toptitle .iscroll").width();if(m>k){$(".toptitle").addClass("hasarrowr");
}}$(".toptitle ul li").click(function(){$(this).addClass("active").siblings().removeClass("active");});if($(this.wrapper).closest(".iscroll").hasClass("plan-scroll")){var n=$(this.wrapper).children()[0].style.top;
$(this.wrapper).closest(".iscroll").siblings(".iscroll").find(".scroller").css("top",n);}}this.x=j;this.y=p;if(this.indicators){for(var l=this.indicators.length;
l--;){this.indicators[l].updatePosition();}}},_initEvents:function(i){var j=i?c.removeEvent:c.addEvent,k=this.options.bindToWrapper?this.wrapper:f;j(f,"orientationchange",this);
j(f,"resize",this);if(this.options.click){j(this.wrapper,"click",this,true);}if(!this.options.disableMouse){j(this.wrapper,"mousedown",this);j(k,"mousemove",this);
...
j(this.scroller,"MSTransitionEnd",this);},getComputedPosition:function(){var j=f.getComputedStyle(this.scroller,null),i,k;if(this.options.useTransform){j=j[c.style.transform].split(")")[0].split(", ");
...
var o=this;this.indicators=[];if(this.options.scrollbars){if(this.options.scrollY){k={el:d("v",l,this.options.scrollbars),interactive:l,defaultScrollbars:true,customStyle:n,resize:this.options.resizeScrollbars,shrink:this.options.shrinkScrollbars,fade:this.options.fadeScrollbars,listenX:false};
this.wrapper.appendChild(k.el);p.push(k);}if(this.options.scrollX){k={el:d("h",l,this.options.scrollbars),interactive:l,defaultScrollbars:true,customStyle:n,resize:this.options.resizeScrollbars,shrink:this.options.shrinkScrollbars,fade:this.options.fadeScrollbars,listenY:false};
...
l=this.currentPage.pageY;if(k>0){n--;}else{if(k<0){n++;}}if(j>0){l--;}else{if(j<0){l++;}}this.goToPage(n,l);return;}n=this.x+e.round(this.hasHorizontalScroll?k:0);
l=this.y+e.round(this.hasVerticalScroll?j:0);if(n>0){n=0;}else{if(n<this.maxScrollX){n=this.maxScrollX;}}if(l>0){l=0;}else{if(l<this.maxScrollY){l=this.maxScrollY;
...
if(!this.wrapperWidth||!this.wrapperHeight||!this.scrollerWidth||!this.scrollerHeight){return;}if(this.options.snap===true){r=e.round(w/2);p=e.round(v/2);
while(u>-this.scrollerWidth){this.pages[s]=[];q=0;t=0;while(t>-this.scrollerHeight){this.pages[s][q]={x:e.max(u,this.maxScrollX),y:e.max(t,this.maxScrollY),width:w,height:v,cx:u-r,cy:t-p};
...
}u=e.max(-j[s].offsetLeft,this.maxScrollX);t=e.max(-j[s].offsetTop,this.maxScrollY);r=u-e.round(j[s].offsetWidth/2);p=t-e.round(j[s].offsetHeight/2);this.pages[o][k]={x:u,y:t,width:j[s].offsetWidth,height:j[s].offsetHeight,cx:r,cy:p};
...
this.snapThresholdY=e.round(this.pages[this.currentPage.pageX][this.currentPage.pageY].height*this.options.snapThreshold);}});this.on("flick",function(){var i=this.options.snapSpeed||e.max(e.max(e.min(e.abs(this.x-this.startX),1000),e.min(e.abs(this.y-this.startY),1000)),300);
...
}var o=0,n=this.pages.length,j=0;if(e.abs(k-this.absStartX)<this.snapThresholdX&&e.abs(p-this.absStartY)<this.snapThresholdY){return this.currentPage;}if(k>0){k=0;
...
}else{if(i<0){i=0;}}if(n>=this.pages[i].length){n=this.pages[i].length-1;}else{if(n<0){n=0;}}var l=this.pages[i][n].x,k=this.pages[i][n].y;j=j===undefined?this.options.snapSpeed||e.max(e.max(e.min(e.abs(l-this.x),1000),e.min(e.abs(k-this.y),1000)),300):j;
...
}}}else{this.options.keyBindings={};}for(j in k){this.options.keyBindings[j]=this.options.keyBindings[j]||k[j];}c.addEvent(f,"keydown",this);this.on("destroy",function(){c.removeEvent(f,"keydown",this);
...
if(this.options.useTransition&&this.isInTransition){p=this.getComputedPosition();this._translate(e.round(p.x),e.round(p.y));this.isInTransition=false;}this.keyAcceleration=k-j<200?e.min(this.keyAcceleration+l,50):0;
...
function k(){var t=c.getTime(),w,v,x;if(t>=p){o.isAnimating=false;var s=$(o.wrapper).find("li").height()*(-1);var u=e.round(q/s);if(q%s!=0){q=s*u;}o._translate(r,q);
if(!o.resetPosition(o.options.bounceTime)){o._execEvent("scrollEnd");}return;}t=(t-j)/l;x=i(t);w=(r-n)*x+n;v=(q-m)*x+m;o._translate(w,v);if(o.isAnimating){h(k);
}}this.isAnimating=true;k();},handleEvent:function(i){switch(i.type){case"touchstart":case"pointerdown":case"MSPointerDown":case"mousedown":this._start(i);
...
break;case"orientationchange":case"resize":this._resize();break;case"transitionend":case"webkitTransitionEnd":case"oTransitionEnd":case"MSTransitionEnd":this._transitionEnd(i);
break;case"wheel":case"DOMMouseScroll":case"mousewheel":this._wheel(i);break;case"keydown":this._key(i);break;case"click":if(!i._constructed){i.preventDefault();
i.stopPropagation();}break;}}};function d(l,j,k){var m=a.createElement("div"),i=a.createElement("div");if(k===true){m.style.cssText="position:absolute;z-index:9999";
...
c.addEvent(f,"touchend",this);}if(!this.options.disablePointer){c.addEvent(this.indicator,c.prefixPointerEvent("pointerdown"),this);c.addEvent(f,c.prefixPointerEvent("pointerup"),this);
}if(!this.options.disableMouse){c.addEvent(this.indicator,"mousedown",this);c.addEvent(f,"mouseup",this);}}if(this.options.fade){this.wrapperStyle[c.style.transform]=this.scroller.translateZ;
...
c.removeEvent(this.indicator,"mousedown",this);c.removeEvent(f,"touchmove",this);c.removeEvent(f,c.prefixPointerEvent("pointermove"),this);c.removeEvent(f,"mousemove",this);
c.removeEvent(f,"touchend",this);c.removeEvent(f,c.prefixPointerEvent("pointerup"),this);c.removeEvent(f,"mouseup",this);}if(this.options.defaultScrollbars){this.wrapper.parentNode.removeChild(this.wrapper);
...
this.lastPointX=i.pageX;this.lastPointY=i.pageY;this.startTime=c.getTime();if(!this.options.disableTouch){c.addEvent(f,"touchmove",this);}if(!this.options.disablePointer){c.addEvent(f,c.prefixPointerEvent("pointermove"),this);
}if(!this.options.disableMouse){c.addEvent(f,"mousemove",this);}this.scroller._execEvent("beforeScrollStart");},_move:function(n){var j=n.touches?n.touches[0]:n,k,i,o,m,l=c.getTime();
...
k.stopPropagation();c.removeEvent(f,"touchmove",this);c.removeEvent(f,c.prefixPointerEvent("pointermove"),this);c.removeEvent(f,"mousemove",this);if(this.scroller.options.snap){var i=this.scroller._nearestSnap(this.scroller.x,this.scroller.y);
var j=this.options.snapSpeed||e.max(e.max(e.min(e.abs(this.scroller.x-i.x),1000),e.min(e.abs(this.scroller.y-i.y),1000)),300);if(this.scroller.x!=i.x||this.scroller.y!=i.y){this.scroller.directionX=0;
...
}else{this.wrapper.style.bottom="2px";}}}var i=this.wrapper.offsetHeight;if(this.options.listenX){this.wrapperWidth=this.wrapper.clientWidth;if(this.options.resize){this.indicatorWidth=e.max(e.round(this.wrapperWidth*this.wrapperWidth/(this.scroller.scrollerWidth||this.wrapperWidth||1)),8);
...
}this.updatePosition();},updatePosition:function(){var i=this.options.listenX&&e.round(this.sizeRatioX*this.scroller.x)||0,j=this.options.listenY&&e.round(this.sizeRatioY*this.scroller.y)||0;
if(!this.options.ignoreBoundaries){if(i<this.minBoundaryX){if(this.options.shrink=="scale"){this.width=e.max(this.indicatorWidth+i,8);this.indicatorStyle.width=this.width+"px";
}i=this.minBoundaryX;}else{if(i>this.maxBoundaryX){if(this.options.shrink=="scale"){this.width=e.max(this.indicatorWidth-(i-this.maxPosX),8);this.indicatorStyle.width=this.width+"px";
...
this.indicatorStyle.width=this.width+"px";}}}if(j<this.minBoundaryY){if(this.options.shrink=="scale"){this.height=e.max(this.indicatorHeight+j*3,8);this.indicatorStyle.height=this.height+"px";
}j=this.minBoundaryY;}else{if(j>this.maxBoundaryY){if(this.options.shrink=="scale"){this.height=e.max(this.indicatorHeight-(j-this.maxPosY)*3,8);this.indicatorStyle.height=this.height+"px";
...
}else{if(j>this.maxPosY){j=this.maxPosY;}}i=this.options.listenX?e.round(i/this.sizeRatioX):this.scroller.x;j=this.options.listenY?e.round(j/this.sizeRatioY):this.scroller.y;
